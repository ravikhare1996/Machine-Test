...
namespace BlazorPWA.Client.Pages.MachineTest.Transaction
{
    ...
    public partial class PurchaseOrderBasePage
    {
        ...
        private void SetSelectedValuesPO_ItemList(EventArgs e)
        {
            try
            {
             if (txtItems?.SelectedValues == null)
             {
              return;
             }
            PO_ItemListValue = txtItems?.SelectedValues?.Cast<FinderData>().ToList();
        ...
        private async Task GetPOItemListFinder(EventArgs args)
        {
            try
            {
                await Task.Run(() =>
                {
                    ...
                    txtItems.CustomFinderParameters = new XSCustomFinderParameters() { ReportID = "POItemList", Query = "Select ICode, IName from tspl_item_Master", CodeColumn = "ICode", NameColumn = "IName", IsCustomFinder = true };
                });
            }
            catch (Exception ex)
            {
